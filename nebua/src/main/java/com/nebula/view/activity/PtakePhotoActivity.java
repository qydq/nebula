...
import com.nebula.take.AndroidTranslucentBar;
 ...
 */
public abstract class PtakePhotoActivity extends BaseActivity implements NetBroadcastReceiver.NetEvevt, DayNightTheme, TakePhoto.TakeResultListener, InvokeListener {
...
//		EMChat.getInstance().init(this.getApplicationContext());
    ...
    public void windowTranslucent() {
        ...
        if (sp.getBoolean("isNight", false)) {
            mWindow.getDecorView().setBackground(ContextCompat.getDrawable(mContext, R.drawable.in_drawable_bgnigt_shape));
        } else {
            mWindow.getDecorView().setBackground(ContextCompat.getDrawable(mContext, R.drawable.in_drawable_bgday_shape));
        }
    }
